...
export interface PaymentMethodSpec {
  ...
  // < 100 makes the checkout's full/partial selector appear for this method
  // (#120). Defaults to 100 (full payment only).
  depositPercent?: number;
}

export interface CourierModalitySpec {
...
export interface CourierSpec {
  ...
  name: string;
  enabled?: boolean;
  modalities: CourierModalitySpec[];
}
...
function kpopCornerStore(): StoreFixtureSpec {
  return {
    ...
    // TRANSFER accepts a 20% advance so the checkout's full/partial selector
    // is exercisable on this demo store (#120). The rest stay full-only.
    paymentMethods: [
      {
        method: 'TRANSFER',
        enabled: true,
        ...
        details: {
          ...
          accountHolder: 'K-Pop Corner',
        },
      },
      {
        method: 'YAPE',
        enabled: true,
        details: { phoneNumber: '+51912345678' },
      },
      {
        method: 'PLIN',
        enabled: true,
    ...
    // Seller-defined couriers with per-modality pricing (#99). Olva offers
    // both modalities, Shalom agency-only, Motorizado home-only.
    couriers: [
      {
        key: 'olva',
        name: 'Olva Courier',
        modalities: [
          { modality: 'AGENCY', price: '8.00' },
          { modality: 'HOME', price: '14.00' },
        ],
      },
      {
        key: 'shalom',
        name: 'Shalom',
        modalities: [{ modality: 'AGENCY', price: '6.50' }],
      },
      {
        key: 'motorizado',
        name: 'Motorizado Lima',
        modalities: [{ modality: 'HOME', price: '12.00' }],
      },
    ],
    pickupPoints: [
    ...
    sections: [
      ...
      },
      {
        ...
        content: {
          title: 'Sobre la tienda',
          body: 'Importamos directo de Corea. Pagos por Yape, Plin o transferencia.',
        },
        position: 2,
      },
    ],
