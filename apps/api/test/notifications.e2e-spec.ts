...
// Validates real notification responses against the schema generated by
// `pnpm --filter api generate:openapi` (apps/api/openapi.json). Notifications
// aren't creatable through the API — this test seeds one directly via
// Prisma (same shape `NotificationsService.createIfNotOpen` writes) to
// exercise findAll/unreadCount/markRead/markAllRead/archive.
...
const notificationSchema = {
  ...openapi.components.schemas.NotificationResponseDto,
  components: openapi.components,
};
const countSchema = {
  ...openapi.components.schemas.NotificationCountResponseDto,
  components: openapi.components,
};
...
describe("notifications (e2e)", () => {
  ...
  const email = `notifications-e2e-${runId}@example.com`;
  const storeSlug = `notifications-e2e-${runId}`;
  ...
  beforeAll(async () => {
    ...
    const notification = await prisma.notification.create({
      data: {
        ...
        type: "LOW_STOCK",
        entityType: "Product",
        entityId: "e2e-product",
        title: "Stock bajo: E2E Product",
        body: "E2E Product tiene 2 unidades disponibles.",
      },
    });
  ...
  it("GET /stores/:storeId/notifications matches the generated openapi.json schema", async () => {
  ...
  it("GET .../unread-count matches the schema", async () => {
    const res = await request(app.getHttpServer())
      .get(`/stores/${storeId}/notifications/unread-count`)
      .set("Cookie", sessionCookie)
      .expect(200);

    assertMatchesSchema(res.body, countSchema, openapi.components);
    expect(res.body.count).toBeGreaterThanOrEqual(1);
  });

  it("PATCH .../:id/read marks it read and matches the schema", async () => {
    const res = await request(app.getHttpServer())
      .patch(`/stores/${storeId}/notifications/${notificationId}/read`)
      .set("Cookie", sessionCookie)
      .expect(200);

    assertMatchesSchema(res.body, notificationSchema, openapi.components);
    expect(res.body.read).toBe(true);
    expect(typeof res.body.readAt).toBe("string");
  });

  it("PATCH .../:id/archive marks it archived and matches the schema", async () => {
    const res = await request(app.getHttpServer())
      .patch(`/stores/${storeId}/notifications/${notificationId}/archive`)
      .set("Cookie", sessionCookie)
      .expect(200);

    assertMatchesSchema(res.body, notificationSchema, openapi.components);
    expect(res.body.archived).toBe(true);
    expect(typeof res.body.archivedAt).toBe("string");
  });

  it("POST .../read-all matches the count schema", async () => {
    const res = await request(app.getHttpServer())
