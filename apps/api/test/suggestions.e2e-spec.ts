...
// Validates real suggestion responses against the schema generated by
// `pnpm --filter api generate:openapi` (apps/api/openapi.json). Suggestions
// are derived read-only signals, not creatable rows — a fresh store with no
// orders/notifications yields an empty array, which is still a valid
// contract check (the endpoint returns `[]`, not an error).
...
const suggestionSchema = {
  ...openapi.components.schemas.SuggestionResponseDto,
...
describe("suggestions (e2e)", () => {
  ...
  const email = `suggestions-e2e-${runId}@example.com`;
  const storeSlug = `suggestions-e2e-${runId}`;
  ...
  it("GET /stores/:storeId/suggestions matches the generated openapi.json schema", async () => {
    const res = await request(app.getHttpServer())
      .get(`/stores/${storeId}/suggestions`)
      .set("Cookie", sessionCookie)
      .expect(200);

    expect(Array.isArray(res.body)).toBe(true);
    for (const suggestion of res.body) {
      assertMatchesSchema(suggestion, suggestionSchema, openapi.components);
    }
  });
});
