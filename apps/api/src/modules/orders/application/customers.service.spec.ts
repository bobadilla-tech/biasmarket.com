...
describe('CustomersService', () => {
  ...
  describe('ownership checks', () => {
    ...
    it('throws ForbiddenException when the user does not own the store', async () => {
      ...
      await expect(service.findAllForStore(storeId, ownerId)).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('findAllForStore', () => {
    ...
    it('counts orders, sums only VERIFIED payments, and tracks the last order date per customer', async () => {
      prisma.customer.findMany.mockResolvedValue([
        { id: 'customer-1', name: 'Ana', phone: '+51987654321', email: 'ana@example.com', emailVerified: true, createdAt: new Date('2026-01-01') },
        { id: 'customer-2', name: null, phone: '+51999999999', email: null, emailVerified: false, createdAt: new Date('2026-01-02') },
      ]);
      prisma.order.groupBy.mockResolvedValue([
        { customerId: 'customer-1', _count: 3, _max: { createdAt: new Date('2026-02-01') } },
        { customerId: 'customer-2', _count: 1, _max: { createdAt: new Date('2026-01-15') } },
      ]);
      ...
      expect(result).toEqual([
        expect.objectContaining({ id: 'customer-1', orderCount: 3, lifetimeSpend: 80 }),
        expect.objectContaining({ id: 'customer-2', orderCount: 1, lifetimeSpend: 0 }),
      ]);
    });

    it('excludes non-VERIFIED payments from lifetime spend', async () => {
      prisma.customer.findMany.mockResolvedValue([
        { id: 'customer-1', name: 'Ana', phone: '+51987654321', email: null, emailVerified: false, createdAt: new Date() },
      ]);
      prisma.order.groupBy.mockResolvedValue([]);
  ...
  describe('findOneForStore', () => {
    ...
    it('throws NotFoundException when the customer belongs to a different store', async () => {
      prisma.customer.findUnique.mockResolvedValue({ id: 'customer-1', storeId: 'other-store' });
      await expect(service.findOneForStore('customer-1', storeId, ownerId)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('returns the customer plus their order history with payment summaries', async () => {
      ...
      const result = await service.findOneForStore('customer-1', storeId, ownerId);

      expect(prisma.order.findMany).toHaveBeenCalledWith(
        expect.objectContaining({ where: { storeId, customerId: 'customer-1' } }),
      );
      expect(result.orders).toEqual([
        expect.objectContaining({ id: 'order-1', paidAmount: 40, pendingAmount: 60, paidPercentage: 40 }),
      ]);
    });
  });
});
