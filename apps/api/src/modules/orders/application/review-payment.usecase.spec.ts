...
describe('ReviewPaymentUseCase', () => {
  ...
  let mailer: { send: Mock<(params: { to: string; subject: string; html: string }) => Promise<{ id: string }>> };
  ...
  beforeEach(async () => {
    ...
    prisma.store.findUnique.mockResolvedValue({ id: storeId, ownerId, name: 'My Store' });
  });
  ...
  it('approve() decrements reserved and stock for finite-stock variants and writes an audit log', async () => {
    ...
    prisma.order.findUniqueOrThrow.mockResolvedValue({ id: orderId, paymentStatus: 'VERIFIED' });
