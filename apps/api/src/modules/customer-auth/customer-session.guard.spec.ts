...
import { CustomerSessionGuard, type CustomerSessionRequest } from './customer-session.guard.js';
...
function buildContext(cookieHeader: string | undefined) {
  const req = { headers: { cookie: cookieHeader } } as unknown as CustomerSessionRequest;
...
describe('CustomerSessionGuard', () => {
  ...
  it('rejects a tampered or expired token', async () => {
    const { context } = buildContext(`${CUSTOMER_SESSION_COOKIE}=not-a-real-token`);
  ...
  it('rejects a token whose password version no longer matches (password changed since issuance)', async () => {
    ...
    prisma.customer.findUnique.mockResolvedValue({ id: 'customer-1', storeId: 'store-1', passwordHash: newHash });
  ...
  it('accepts a valid token, attaches the session, and reissues a fresh cookie', async () => {
    ...
    prisma.customer.findUnique.mockResolvedValue({ id: 'customer-1', storeId: 'store-1', passwordHash });

    const { req, res, cookieMock, context } = buildContext(`${CUSTOMER_SESSION_COOKIE}=${token}`);

    const result = await guard.canActivate(context);

    expect(result).toBe(true);
    expect(req.customerSession).toEqual({ id: 'customer-1', storeId: 'store-1' });
  ...
  it('rejects when the token belongs to a customer that has since moved stores', async () => {
    ...
    prisma.customer.findUnique.mockResolvedValue({ id: 'customer-1', storeId: 'store-2', passwordHash });
