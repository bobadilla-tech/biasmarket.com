...
describe('SuggestionsService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [SuggestionsService, { provide: PrismaService, useValue: prisma }],
    }).compile();
  ...
  describe('ownership checks', () => {
    it('throws NotFoundException when the store does not exist', async () => {
      ...
      await expect(service.getSuggestions(storeId, ownerId)).rejects.toThrow(NotFoundException);
    });
    ...
    it('throws ForbiddenException when the user does not own the store', async () => {
      ...
      await expect(service.getSuggestions(storeId, ownerId)).rejects.toThrow(ForbiddenException);
    });
  });

  describe('getSuggestions', () => {
    beforeEach(() => {
      prisma.store.findUnique.mockResolvedValue({ id: storeId, ownerId, holdWindowHours: 48 });
    });
    ...
    it('surfaces the low-stock, stale-orders, and top-seller suggestions together', async () => {
      ...
      prisma.orderItem.groupBy.mockResolvedValue([{ productId: 'product-1', _sum: { quantity: 20 } }]);
