...
describe('StatsService', () => {
  ...
  describe('ownership checks', () => {
    it('throws NotFoundException when the store does not exist', async () => {
      ...
      await expect(service.getOverview(storeId, ownerId)).rejects.toThrow(NotFoundException);
    });
    ...
    it('throws ForbiddenException when the user does not own the store', async () => {
      ...
      await expect(service.getOverview(storeId, ownerId)).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('getOverview', () => {
    beforeEach(() => {
      stubOwnedStore();
      prisma.orderPayment.aggregate.mockResolvedValue({ _sum: { amount: 150 } });
