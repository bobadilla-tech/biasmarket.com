...
import { lowStockRule, noRecentOrdersRule, staleOrdersRule, topSellerRule } from './suggestion-rules.js';
...
export class SuggestionsService {
  ...
  async getSuggestions(storeId: string, userId: string): Promise<Suggestion[]> {
    ...
    const staleCutoff = new Date(Date.now() - store.holdWindowHours * 60 * 60 * 1000);
    const recentCutoff = new Date(Date.now() - RECENT_ORDERS_WINDOW_DAYS * 24 * 60 * 60 * 1000);

    const [lowStockCount, staleOrderCount, recentOrderCount, topProductRows] = await Promise.all([
      ...
      this.prisma.order.count({ where: { storeId, createdAt: { gte: recentCutoff } } }),
