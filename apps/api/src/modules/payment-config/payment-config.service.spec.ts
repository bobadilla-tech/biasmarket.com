...
describe('PaymentConfigService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [PaymentConfigService, { provide: PrismaService, useValue: prisma }],
    }).compile();
  ...
  describe('findEnabledForStore', () => {
    it('only returns enabled methods', async () => {
      ...
      prisma.paymentMethodConfig.findMany.mockResolvedValue([{ method: 'YAPE', enabled: true }]);
  ...
  describe('upsert', () => {
    ...
    it('toggles enabled on an existing method without touching details', async () => {
      ...
      await service.upsert(storeId, ownerId, { method: 'PLIN', enabled: false });
