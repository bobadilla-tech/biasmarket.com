...
describe('NotificationsService', () => {
  ...
  describe('ownership checks', () => {
    ...
    it('throws ForbiddenException when the user does not own the store', async () => {
      ...
      await expect(service.unreadCount(storeId, ownerId)).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('findAllForStore', () => {
    it('scopes the query by storeId and applies filters', async () => {
      ...
      await service.findAllForStore(storeId, ownerId, { archived: false, read: true });
  ...
  describe('markRead / archive', () => {
    ...
    it('throws NotFoundException when the notification belongs to a different store', async () => {
      ...
      await expect(service.markRead(notificationId, storeId, ownerId)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('resolveOpenStockAlerts', () => {
    it('archives open LOW_STOCK/OUT_OF_STOCK rows for the entity', async () => {
      await service.resolveOpenStockAlerts(storeId, 'ProductVariant', 'variant-1');
  ...
  describe('syncStockAlerts', () => {
    const store = { id: storeId, lowStockThreshold: 3, lowStockAlertsEnabled: true };
    ...
    it('creates OUT_OF_STOCK when available stock is zero or negative', async () => {
      prisma.notification.findFirst.mockResolvedValue(null);
      prisma.productVariant.findMany.mockResolvedValue([{ stock: 5, reserved: 5 }]);
      ...
      expect(prisma.notification.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ type: 'OUT_OF_STOCK', entityType: 'ProductVariant' }),
        }),
      );
    });
    ...
    it('creates LOW_STOCK when available stock is at or below the threshold', async () => {
      prisma.notification.findFirst.mockResolvedValue(null);
      prisma.productVariant.findMany.mockResolvedValue([{ stock: 5, reserved: 3 }]);
      ...
      expect(prisma.notification.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ type: 'LOW_STOCK', entityType: 'ProductVariant' }),
        }),
      );
    });

    it('resolves open alerts when available stock rises above the threshold', async () => {
      prisma.productVariant.findMany.mockResolvedValue([{ stock: 20, reserved: 0 }]);
      ...
      expect(prisma.notification.updateMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({ entityType: 'ProductVariant', entityId: 'variant-1' }),
        }),
      );
    });

    it('only resolves (never creates) for unlimited-stock variants', async () => {
      prisma.productVariant.findMany.mockResolvedValue([{ stock: null, reserved: 0 }]);
