...
import { buildWhatsAppPaymentReminderMessage, buildWhatsAppUrl } from "@biasmarket/utils/whatsapp";
...
const PAYMENT_ATTENTION_STATUSES = new Set(["PENDING_PAYMENT", "PARTIALLY_PAID", "PAYMENT_SUBMITTED"]);
...
export default function PaymentsPage() {
  ...
  const paymentOrders = useMemo(
    () => (orders ?? []).filter((order) => PAYMENT_ATTENTION_STATUSES.has(order.paymentStatus)),
    [orders],
  );
  ...
  return (
    <div className="min-h-screen px-5 py-6 lg:px-8 lg:py-8">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[#f3ebff] bg-[#fcf9ff] text-left text-[11px] font-semibold uppercase tracking-[0.18em] text-[#927fac]">
                    ...
                    <th className="px-6 py-4">{tOrders("columns.customer")}</th>
                    ...
                    <th className="px-6 py-4 text-right">{tOrders("columns.actions")}</th>
                  </tr>
                </thead>
                <tbody>
                  {paymentOrders.map((order) => {
                    ...
                    const date = formatOrderDate(order.createdAt, locale, tOrders);
                    const needsReview =
                      order.paymentStatus === "PENDING_PAYMENT" || order.paymentStatus === "PAYMENT_SUBMITTED";

                    return (
                      <tr key={order.id} className="border-b border-[#f3ebff] last:border-0 hover:bg-[#fcf9ff]">
                          ...
                          <div className="flex flex-wrap items-center justify-end gap-1.5">
                            {needsReview && (
                              <>
                                <Button
                                  type="button"
                                  onClick={() => setConfirmTarget({ orderId: order.id, decision: "approve" })}
                                ...
                                <Button
                                  type="button"
                                  variant="outline"
                                  onClick={() => setConfirmTarget({ orderId: order.id, decision: "reject" })}
                            ...
                            {order.paymentStatus === "PARTIALLY_PAID" && (
                              <a href={contactBuyerUrl(order)} target="_blank" rel="noopener noreferrer">
                                <Button
                                  type="button"
                                  variant="outline"
      ...
      <ConfirmTransitionDialog
        open={!!confirmTarget}
        label={confirmTarget ? paymentActionLabels[confirmTarget.decision] : null}
