...
import { useStatsOverview, StatTile, RecentOrdersList } from "@/features/stats";
import type { FulfillmentStatusValue, PaymentStatusValue } from "@/features/stats";
    ...
    <div className="min-h-screen px-5 py-6 lg:px-8 lg:py-8">
      <div className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          />
          <StatTile icon={Clock} label={t("stats.pendingReview")} value={String(pendingReview)} />
          <StatTile
            icon={AlertTriangle}
        ...
        <ul className="space-y-2.5">
          {(Object.keys(counts) as PaymentStatusValue[]).map((status) => (
            <li key={status} className="flex items-center justify-between text-sm">
              ...
              <span className="font-semibold text-[#2d1649]">{counts[status]}</span>
            </li>
          ))}
        </ul>
      </CardContent>
