...
import { OrderStatusBadge, formatOrderDate } from "@/features/orders";
...
export function CustomerDetailSheet({
  ...
  const { data, isPending } = useCustomer(storeId, customerId, tCommon("networkError"));
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="h-dvh w-[420px] gap-0 overflow-y-auto sm:max-w-[420px]">
        {isPending ? (
          ...
          <>
            <SheetHeader>
              <SheetTitle>{data.customer.name ?? data.customer.phone}</SheetTitle>
