...
import { customerRegisterSchema, type CustomerRegisterInput } from "../schemas/register.schema";
...
export function SetPasswordForm({ slug, token }: { slug: string; token: string }) {
  ...
  const onSubmit = async (values: CustomerRegisterInput) => {
    try {
      ...
      setError("root", { message: err instanceof Error ? err.message : undefined });
    }
  };
  ...
  if (success) {
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col gap-3">
        ...
        <Link href={`/store/${slug}/account/login`} className="store-theme-link font-semibold text-center">
          {t("goToLogin")}
        </Link>
      </div>
    );
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
        <div className="flex flex-col gap-1.5">
          <input
            ...
            {...register("password")}
          />
          {errors.password ? <p className="text-sm text-red-500">{t("passwordTooShort")}</p> : null}
        </div>
        ...
        {errors.root ? (
          <p className="text-sm text-red-500">{errors.root.message ?? t("genericError")}</p>
        ) : null}
