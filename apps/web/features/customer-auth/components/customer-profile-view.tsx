...
export function CustomerProfileView({ slug, profile }: { slug: string; profile: CustomerProfile }) {
      ...
      <div className="max-w-md mx-auto flex flex-col gap-6">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">{profile.customer.phone}</p>
          </div>
          <button
            type="button"
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col gap-2">
          <h2 className="text-sm font-semibold text-gray-900">{t("infoTitle")}</h2>
          <p className="text-sm text-gray-600">{profile.customer.name ?? t("noName")}</p>
          <p className="text-sm text-gray-600">{profile.customer.email ?? t("noEmail")}</p>
        </div>
