...
test("list validates the response against collectionListSchema", async () => {
  ...
  expect(apiFetch).toHaveBeenCalledWith("/stores/store-1/collections", {}, undefined);
...
test("create omits an empty description", async () => {
  ...
  await collectionsApi.create("store-1", { name: "Photocards", description: "" });

  expect(apiFetch).toHaveBeenCalledWith(
    "/stores/store-1/collections",
    { method: "POST", body: JSON.stringify({ name: "Photocards", description: undefined }) },
    undefined,
  );
});
