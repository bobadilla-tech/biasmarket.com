...
import { getOrderNumber, getInitials, formatOrderDate, getDeliveryLabel, getProductSummary } from "../lib/order-format";
...
export function OrdersTable({
  ...
  if (orders.length === 0) {
    return <div className="px-6 py-10 text-sm text-[#8f7da8]">{t("empty")}</div>;
  }
              ...
              <tr key={order.id} className="border-b border-[#f3ebff] last:border-0 hover:bg-[#fcf9ff]">
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    {avatar ? (
                      <img className="size-9 rounded-full object-cover" src={avatar} alt={customer} />
                    ) : (
                      <div
