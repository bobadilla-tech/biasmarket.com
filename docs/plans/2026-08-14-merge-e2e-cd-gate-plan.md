...
(22 vs. 24 E2E specs, pre-`postgres:18` image digests) and its scope was
trimmed — the successor drops the `merge-policy.yml` skip-CI-token workflow,
the `CODEOWNERS` rewrite, the `release-config` path-filter job, and
`merge_group` support as orthogonal hardening.
