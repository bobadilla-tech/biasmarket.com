...
- [x] Phase 0. Credentials collected; all four Contabo credentials verified
      against the token endpoint (the API password was set via _Send link_).
...
- [x] Phase 4. `contabo_secret` + cloud-init `user_data`. Reinstall applied
      2026-09-21 (empty box). Verified over SSH with the pinned host key: Docker
      29.8.1, `deploy` user, `ufw` 22/80/443, 4 GB swap, `/opt/biasmarket`,
      `rrsync` symlink, password login off. cloud-init reports `error` only
      because of Contabo's own `bootcmd` (`pkill -HUP sshd` fails on 26.04);
      none of our steps failed.
- [x] Phase 5. Deploy keys and the host key generated by `tls_private_key`
      (option B). Private keys live in local state and in `user_data`; state
      must be treated as a secret.
- [x] Phase 7. GitHub `production` environment + 5 `NEXT_PUBLIC_*` variables
      imported, 5 deploy secrets written by Terraform (`No changes` after).
...
- **Data-migration gate: satisfied by explicit approval.** The maintainer
  confirmed on 2026-09-20 that the data on Oracle is OK to lose, so this is a
  fresh start with no `pg_dump`/`mc mirror` step. Oracle being unreachable is
  _not_ evidence that no data existed and is not the basis for this decision.
  The first deploy (Phase 9) stays gated on this approval; if it is withdrawn,
  stop and recover a backup before bootstrapping.
...
- Remote state (Cloudflare R2 is S3-compatible, so the `s3` backend works) with
  state locking discussed and encryption. Only after state is shared and
  migrated can `infra/terraform/imports.tf` be deleted: until then state is
  local and gitignored, so a fresh clone has none, and without the import blocks
  `terraform plan` would try to _create_ a second VPS.
...
- **Data on Oracle.** Resolved: the maintainer explicitly accepted losing it
  (see the data-migration gate under _Facts gathered_). The Phase 4 reinstall
  only ever wiped the empty Contabo box, never Oracle data.
