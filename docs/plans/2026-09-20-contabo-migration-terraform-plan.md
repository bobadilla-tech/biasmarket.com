...
- [x] Phase 4. `contabo_secret` + cloud-init `user_data`. Reinstall applied
      2026-09-21 (empty box). Verified over SSH with the pinned host key:
      Docker 29.8.1, `deploy` user, `ufw` 22/80/443, 4 GB swap,
      `/opt/biasmarket`, `rrsync` symlink, password login off. cloud-init
      reports `error` only because of Contabo's own `bootcmd` (`pkill -HUP
    sshd` fails on 26.04); none of our steps failed.
...
- [ ] Phase 6 (cutover half: repoint DNS), 9 (first deploy), 10.
