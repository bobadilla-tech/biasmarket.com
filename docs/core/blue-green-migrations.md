...
Changing `user_data`, `image_id` or the SSH keys on the instance **reinstalls the
server**, see the guide. The numbered steps below stay as the manual reference
(for another provider, or to understand what cloud-init does); they were written
for Ubuntu 24.04 on Oracle Ampere A1 (arm64) and the 26.04 / x86_64 differences
are noted where they matter. Run manual steps in order; each depends on the one
...
Capture once, **verified out-of-band via your provider's console** (Contabo: the VNC console
in the customer panel; not needed on the Terraform path, where the host key is
generated by Terraform and known in advance) (not blind
...
The first production bootstrap (on Oracle, before the move to Contabo) exposed a few details worth making explicit:
